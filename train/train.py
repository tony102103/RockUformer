...
import argparse
import options
opt = options.Options().init(argparse.ArgumentParser(description='Image deblurring')).parse_args()
import utils
...
def main():
    ######### Logs dir ###########
    log_dir = os.path.join(opt.save_dir, 'deblur', opt.dataset, opt.arch + opt.env)
    ...
    safe_time_str = current_time.replace(':', '-')
    logname = os.path.join(log_dir, safe_time_str + '.txt')
    ...
    if opt.warmup:
        ...
        scheduler_cosine = CosineAnnealingLR(optimizer, opt.nepoch - warmup_epochs, eta_min=1e-6)
    ...
    print("Number of training batches per epoch: ", num_train_batches, "\t",  "Number of validation batches per epoch: ", num_val_batches)
        ...
        for i, data in enumerate(tqdm(train_loader), 0):
            ...
            if (i + 1) % eval_now == 0 and i > 0:
                with torch.no_grad():
                    ...
                    print(
                        "[Ep %d it %d\t PSNR ROCK: %.4f\t] ----  [best_Ep_ROCK %d best_it_ROCK %d Best_PSNR_ROCK %.4f] " % (
